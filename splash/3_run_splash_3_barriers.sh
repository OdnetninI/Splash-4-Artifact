...
echo -n "" > Splash-3-Barriers.times

for i in $(find Splash-3-Barriers -executable -type f | sort); do
    ...
    for cores in ${_cores}; do
        ...
        echo -n "Splash-3-Barriers,${cores},${bin}," | tee -a ${currentDIR}/Splash-3-Barriers.times

        for p in {0..9}; do
            eval "./${bin} ${args}" 2>&1 >> ${currentDIR}/outputs/${ID}.out
        done

        grep ROI ${currentDIR}/outputs/${ID}.out  | cut -f 6 -d " " | tr -d "s" | tr "\n" "," | tee -a ${currentDIR}/Splash-3-Barriers.times

        echo "" | tee -a ${currentDIR}/Splash-3-Barriers.times
